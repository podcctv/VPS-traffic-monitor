...
<div class="grid lg:grid-cols-2 gap-4"><div v-for="row in rows" :key="row.node_id" class="rounded-2xl p-5 bg-slate-900/80 border border-cyan-500/30 shadow-lg space-y-3"><div class="flex items-start justify-between gap-2"><div><h3 class="text-xl font-bold">{{ row.node_id }}</h3><p class="text-xs text-slate-400 mt-1">计费网卡：{{ row.billing_iface }}</p></div><span class="text-xs px-2 py-1 rounded bg-cyan-400/20 text-cyan-200">{{ row.report_iface }}</span></div><p class="text-3xl font-black text-emerald-300">{{ row.used }}</p><p class="text-slate-400">月配额 {{ row.monthly_quota_gb }} GB · 重置日 {{ row.reset_day }}</p><div class="h-2 rounded bg-slate-700"><div class="h-2 rounded bg-gradient-to-r from-cyan-400 to-fuchsia-500" :style="{width: row.percent + '%'}"></div></div><div v-if="row.interfaces.length" class="rounded-xl border border-slate-700/80 p-3"><p class="text-xs text-slate-400 mb-2">网卡监控明细（主网卡用于配额，子网卡仅展示）</p><div class="grid grid-cols-1 sm:grid-cols-2 gap-2"><div v-for="iface in row.interfaces" :key="iface.name" class="bg-slate-950/70 border border-slate-700 rounded-lg px-3 py-2"><p class="text-xs text-slate-400">{{ iface.name }}</p><p class="text-sm font-bold" :class="iface.is_billing?'text-emerald-300':'text-cyan-300'">{{ iface.total_gib.toFixed(2) }} GiB</p></div></div></div><p class="text-xs text-slate-500">最后上报：{{ row.timestamp }}</p></div><div v-if="rows.length===0" class="rounded-2xl p-6 border border-dashed border-slate-600 text-slate-400">暂无数据，请稍后刷新。</div></div></div>
<script>const {createApp}=Vue;createApp({data(){return{rows:[],stats:{nodes:0,total:'0 GiB',latest:'-'}}},methods:{async loadBoard(){const res=await fetch('/api/v1/public-dashboard');if(!res.ok){this.rows=[];return;}const data=await res.json();let sum=0,latest='-';this.rows=(data.nodes||[]).map(n=>{const li=(data.latest_ingest||{})[n.node_id]||{};const counters=li.counters||{};const totalFromCounters=(Number(counters.rx_total_bytes||0)+Number(counters.tx_total_bytes||0))/1024/1024/1024;const interfaces=(li.interfaces||[]).map(i=>{const ic=i.counters||{};const total=(Number(ic.rx_total_bytes||0)+Number(ic.tx_total_bytes||0))/1024/1024/1024;return {name:i.name||'-',total_gib:total,rx_gib:Number(ic.rx_total_bytes||0)/1024/1024/1024,tx_gib:Number(ic.tx_total_bytes||0)/1024/1024/1024};});const reportedIface=li.iface||'';const ifaceNames=interfaces.map(i=>i.name).filter(Boolean);const configuredIface=(n.agent_iface&&n.agent_iface!=='all')?n.agent_iface:'';const billingIface=(configuredIface&&ifaceNames.includes(configuredIface))?configuredIface:(reportedIface&&ifaceNames.includes(reportedIface))?reportedIface:(ifaceNames[0]||configuredIface||reportedIface||'all');const billingStat=interfaces.find(i=>i.name===billingIface);const used=billingStat?billingStat.total_gib:totalFromCounters;sum+=used;if(li.timestamp&&li.timestamp>latest){latest=li.timestamp;}const percent=Math.min(100,Math.round((used/Math.max(1,n.monthly_quota_gb))*100));return {...n,used:`${used.toFixed(2)} GiB`,billing_iface:billingIface,report_iface:reportedIface||billingIface||'-',timestamp:li.timestamp||'-',percent,interfaces:interfaces.map(i=>({...i,is_billing:i.name===billingIface}))};});this.stats={nodes:this.rows.length,total:`${sum.toFixed(2)} GiB`,latest};}},async mounted(){await this.loadBoard();setInterval(this.loadBoard,15000);}}).mount('#app');</script></body></html>"""
